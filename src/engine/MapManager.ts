...
export const isTileAccessible = (x: number, y: number) => {
  ...
  if (tileType === "l" && !gameState.player.inventory.hasItemBy("redStone")) {
    ...
    tileType === "w" &&
    !gameState.player.inventory.hasItemBy("blueStone")
  ) {
    return false;
  } else if (
    ["T", "║", "═", "╝", "╗", "╔", "╚", "╩", "╦", "╠", "╣", "╬", "■"].includes(
...
export const isTileIsNotObstructed = (x: number, y: number) => {
  ...
  if (
    itemAtPos &&
    itemAtPos instanceof AbstractTalkablePlayerItem &&
    !itemAtPos.isWalkable()
  ) {
    return false;
...
export const getItemAtPosition = (position: Position) => {
  return gameState.mapStates[gameState.currentMap].items.getItemByPosition(
    position,
  );
};
...
const getPositionInFrontOfPlayer = () => {
  ...
  if (
    frontPositionX < 0 ||
    frontPositionX >= getCurrentMap().grid.getWidth() ||
    frontPositionY < 0 ||
    frontPositionY >= getCurrentMap().grid.getHeight()
  ) {
    return undefined;
  }
