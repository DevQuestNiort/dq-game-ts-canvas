...
const paintDeathView = () => {
  if (gameState.player.isDead()) {
    ...
    pCtx.fillText(
      "GAME OVER",
      (GRID_PITCH * VIEWPORT_SIZE_X) / 2,
      (GRID_PITCH * VIEWPORT_SIZE_Y) / 2,
      GRID_PITCH * 23 - 10,
    );
    drawDialog(
      new Position(0, 0),
      new TwoDimensionalSize(TOTAL_PX_SIZE_X, TOTAL_PX_SIZE_Y),
      patternCanvas,
    );
  }
};

export const paintMenuFullPage = () => {
  if (gameState.view !== viewEnum.MAP) {
    drawDialog(
      new Position(0, 0),
      new TwoDimensionalSize(TOTAL_PX_SIZE_X, TOTAL_PX_SIZE_Y),
      gameState.getCurrentView().paint(),
    );
  }
};
...
const paintPlayerDialog = () => {
  ...
  drawDialog(
    new Position(0, GRID_PITCH * VIEWPORT_SIZE_Y),
    new TwoDimensionalSize((GRID_PITCH * VIEWPORT_SIZE_X) / 2, GRID_PITCH * 3),
    patternCanvas,
  );
};
...
const paintInteractionDialog = () => {
  ...
  if (itemInFrontOfPlayer && itemInFrontOfPlayer instanceof PNJItem) {
    ...
    if (itemInFrontOfPlayer.underAttack) {
      ...
      pCtx.textAlign = "end";
      pCtx.fillText(
        "Appuyer sur R pour parler ou F pour attaquer ",
        (GRID_PITCH * VIEWPORT_SIZE_X) / 2,
        84,
        500,
      );
    ...
    itemInFrontOfPlayer.interaction
  ) {
    pCtx.fillStyle = "#fff";
...
let itemAtPlayerPositionChanged = true;
export const notifyItemAtPlayerPositionChanged = () => {
  itemAtPlayerPositionChanged = true;
};
...
const paintPopupItemInfoDialog = () => {
  if (itemAtPlayerPositionChanged) {
    ...
    if (itemAtPlayerPosition) {
      ...
      drawDialog(
        new Position(0, 0),
        new TwoDimensionalSize(GRID_PITCH * 19, GRID_PITCH * 3),
        patternCanvas,
      );
    }
    itemAtPlayerPositionChanged = false;
  }
};
