...
export class MainMenuState extends MenuState {
  ...
  constructor() {
    ...
    const entry1 = new IhmEntry(
      "01",
      new Position((GRID_PITCH * VIEWPORT_SIZE_X) / 2, GRID_PITCH * 8),
      30,
      "Nouvelle partie",
      () => newGame(),
    );
    ...
    this.texts = [
      ...
      new IhmEntry(
        "subtitle",
        new Position((GRID_PITCH * VIEWPORT_SIZE_X) / 2, GRID_PITCH * 3),
        20,
        "Le jeu officiel de la conférence officielle de Niort",
        undefined,
      ),
    ];
  }
  ...
  paint() {
    ...
    this.entrys.forEach((entry, index) => {
      ...
      if (index === this.selectedEntry) {
        ...
        pCtx.drawImage(
          getImage("logodq"),
          entry.position.x - textWidth / 2 - GRID_PITCH * 2.0,
          entry.position.y,
          GRID_PITCH * 1.5,
          GRID_PITCH * 1.5,
        );
