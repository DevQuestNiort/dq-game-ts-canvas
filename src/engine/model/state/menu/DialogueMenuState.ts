...
export class DialogueMenuState extends MenuState {
  ...
  constructor() {
    ...
    const entry1 = new IhmEntry(
      "01",
      new Position((GRID_PITCH * VIEWPORT_SIZE_X) / 2, GRID_PITCH * 20),
      20,
      "Retour au jeu",
      () => {
  ...
  setTemplate(modalTemplate: ModalTemplate) {
    ...
    const choicesEntrys = modalTemplate.choices.map(
      (choice, index) =>
        new IhmEntry(
          choice.titre,
          new Position(
            (GRID_PITCH * VIEWPORT_SIZE_X) / 2,
            GRID_PITCH * (20 - nuberOfEntry * 1 + (index - 1) * 1),
          ),
          20,
          choice.titre,
          choice.action,
  ...
  paint() {
    ...
    splittext(pCtx, text, maxlength).forEach((text, index) => {
      ...
      pCtx.fillText(
        text.trim(),
        GRID_PITCH * 3,
        GRID_PITCH * 4 + (index + 1) * 25,
        TOTAL_PX_SIZE_X - GRID_PITCH * 6,
      );
    });
