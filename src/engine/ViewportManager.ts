...
export const centerViewportOnPlayer = () => {
    ...
    setViewportPositionWithinMapY(gameState.player.position.y - Math.floor(gameConfiguration.viewport.dimension.height / 2));
}
...
const setViewportPositionWithinMapX = (x: number) => {
    ...
    if (viewportX + gameConfiguration.viewport.dimension.width > getCurrentMap().grid.getWidth()) {
        viewportX = getCurrentMap().grid.getWidth() - gameConfiguration.viewport.dimension.width;
    }
...
const setViewportPositionWithinMapY = (y: number) => {
    ...
    if (viewportY + gameConfiguration.viewport.dimension.height > getCurrentMap().grid.getHeight()) {
        viewportY = getCurrentMap().grid.getHeight() - gameConfiguration.viewport.dimension.height;
    }
