...
export const paintItemsLayer = (
  ...
  items.get().forEach((item) => {
    if (item instanceof TrapItem && !item.isvisible()) {
      ...
      if (
        item.position.x >= gameState.viewport.position.x &&
        item.position.x <
          gameState.viewport.position.x +
            gameConfiguration.viewport.dimension.width &&
        item.position.y >= gameState.viewport.position.y &&
        item.position.y <
        ...
        if (
          viewportChanged ||
          tilesChanged.find(
            (tile) => tile.x === item.position.x && tile.y === item.position.y,
          )
        ) {
