...
import {
  getFiller,
  init as initGridBackgroundFillers,
} from "./GridBackgroundFillers.ts";
...
export const paintBackground = (
  ...
  viewportDimension: TwoDimensionalSize,
  ...
  if (viewportChanged) {
    for (let x = 0; x < viewportDimension.width; x++) {
      for (let y = 0; y < viewportDimension.height; y++) {
        paintBackgroundTile(
          ...
          y,
          grid.getCase(
            viewportState.position.x + x,
            viewportState.position.y + y,
          ),
        );
      }
    }
    ...
    for (let tile of tilesChanged) {
      paintBackgroundTile(
        tile.x - viewportState.position.x,
        tile.y - viewportState.position.y,
        grid.getCase(tile.x, tile.y),
      );
    }
  }
  ...
  if (getBooleanOption(BooleanOption.DEBUG_MODE)) {
    ...
    canvasContext.strokeRect(
      gameConfiguration.viewport.deadZone.position.x * GRID_PITCH,
