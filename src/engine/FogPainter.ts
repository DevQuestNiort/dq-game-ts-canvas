  ...
  if (fog !== undefined && !getBooleanOption(BooleanOption.DEBUG_MODE)) {
    if (viewportChanged) {
      ...
      for (let tile of tilesChanged) {
        paintFogTile(
          tile.x - gameState.viewport.position.x,
          tile.y - gameState.viewport.position.y,
          fog,
        );
      }
    }
...
const paintFogTile = (x: number, y: number, fog: Fog) => {
  const distanceX = Math.abs(
    x + gameState.viewport.position.x - gameState.player.position.x,
  );
  const distanceY = Math.abs(
    y + gameState.viewport.position.y - gameState.player.position.y,
  );
